    ...
    while True:
        ...
        for vehicle in controllable_vehicles:
            ...
            if action_vehicle in action_map:
                ...
                actions.append(1)  # Default: IDLE, equivalent to actions.append(action_map["IDLE"])
        actions_tuple = tuple(actions)

        obs, reward, done, truncated, info = env.step(actions_tuple)  # (1, 2) -- v1="IDLE" v2="LANE_RIGHT"
        step_count += 1
...
def fall_behind(behind_vehicle: ControllableVehicle, in_front_vehicle: Vehicle, min_distance = 25.0, max_duration = float("inf")):
    global step_count
    step_count_t0 = step_count
...
def change_to_same_lane(vehicle_to_change_lane: ControllableVehicle, other_vehicle: Vehicle):
...
def close_distance(behind_vehicle: ControllableVehicle, in_front_vehicle: Vehicle, max_distance = 25.0, max_duration = float("inf")):
    global step_count
    step_count_t0 = step_count
...
def equalize_speeds(controllable_vehicle: ControllableVehicle, other_vehicle: Vehicle):
    while (abs(controllable_vehicle.target_speed() - other_vehicle.target_speed()) <= 0.1
...
@thread
def follow_behind(behind_vehicle : ControllableVehicle, in_front_vehicle: Vehicle, delay_seconds : float = 0.0):
    # " serial: "
...
def abstract_scenario_2():
    # cond 1.
    satisfied = yield from await_condition(lambda : v1_red.is_behind(vut), local_reward=10.0)
    if not satisfied:
        print("################ UNSAT")
        return
    else:
    ...
    if not satisfied:
        ...
        print("################ COND 2 SAT")
    # cond 3.
    satisfied = yield from await_condition(lambda : v2_green.is_behind(v1_red), local_reward=10.0)
    if not satisfied:
        print("################ UNSAT")
        return
    else:
...
def parallel(*bthreads):
    ...
    yield sync(waitFor=true)  # needs to be here, otherwise there might be problem with no b_thread syncing anything.


if __name__ == "__main__":
    # Creating a BProgram with the defined b-threads, SMTEventSelectionStrategy,
    # and a listener to print the selected events
    b_program = BProgram(bthreads=[highway_env_bthread(),
