...
def decide_action(env, obs, obs_wrapper: ObservationWrapper, v_id: int, space: float) -> int:
    ...
    if istuple and not isIDM:
        return define_multiagent_action(env, obs_wrapper, v_id, space)
    else:
        return define_action_nonagent_vut(env, obs_wrapper, v_id, space)


def define_multiagent_action(env, obs_wrapper: ObservationWrapper, v_id: int, space: float) -> int:
    """
        Decides the action to be taken by the vehicle based on the current observation.
        :param env: the highway environment
        :param obs_wrapper: The observation wrapper containing functionality based on the current observation
...
def define_action_nonagent_vut(env, obs_wrapper: ObservationWrapper, v_id: int, space: float) -> int:
    """
        Decides the action to be taken by the vehicle based on the current observation.
        :param env: the highway environment
        :param obs_wrapper: The observation wrapper containing functionality based on the current observation
    ...
    if distance > 35 and road_neighbours[0] is not None:
        ...
        return 1
    elif distance == 0 and right_clear and current_lane - other_lane_info < 0 and current_lane in [2, 3] and current_lane == 2:
        return 2
    elif distance == 0 and right_clear and current_lane - other_lane_info > 0 and current_lane in [2, 3] and current_lane == 3:
        return 0
