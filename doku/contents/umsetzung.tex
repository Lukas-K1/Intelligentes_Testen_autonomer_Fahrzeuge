...
\section{Visualisierung von ausgeführten konkreten Szenarien}
...
\lstinputlisting[language=json, caption={Beispiel-Log Schichten}, label={lst:demo-json-layers}]{contents/code/layers_config.json}
...
Im Beispiel zuletzt wird zu jedem Eintrag ein \texttt{layer} geloggt, welches die Zuordnung zu den oben erläuterten Schichten herstellt. Werden allerdings keine Schichten als \texttt{layers}-Attribut geloggt, werden die hier in den einzelnen Einträgen benannten Schichten genutzt, um Schichten zu identifizieren, in denen die Balken angezeigt werden. Dabei wird auch eine Hierarchie inferiert, die jedoch von den gewünschten Hierarchie abweichen kann, weshalb empfahlen wird, die Schichten explizit zu definieren.
...
Neben den zwingend notwendigen Attributen der Log-Einträge können frei weitere Informationen geloggt werden. Im Listing \ref{lst:demo-json} ist zu den letzten drei Einträgen (ab Zeile 10) zusätzlich \texttt{distance\_to\_vut} geloggt. Diese Daten werden genutzt, um Diagramme wie in Abbildung \ref{fig:analyzer_diagram_panel} zu erstellen, das im \enquote{Diagram Panel} angezeigt wird. Dabei kann der Nutzer die jeweiligen Attribute beliebig benennen. Zu einem Attribut (hier \texttt{distance\_to\_vut}) werden dann alle geloggten Werte in einem Diagramm nach Akteuren unterteilt eingetragen.
...
Ein besonderer Vorteil des entwickelten Analysetools ist, dass es durch die Entwicklung ausschließlich mit HTML, CSS und JavaScript jederzeit auf dem Computer des Entwicklers mithilfe eines beliebigen Browsers eingesetzt werden kann. Da es als Teil des Projekts abgelegt wurde, ist es während der Arbeit am Projekt stets verfügbar. Darüber hinaus können erstellte Analysen über den Export (oben rechts in Abbildung \ref{fig:analyzer_graph}) exportiert und mit anderen geteilt werden.
